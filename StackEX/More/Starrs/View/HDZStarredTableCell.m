...
@end
@implementation HDZStarredTableCell
...
- (void)configureWithModel:( HDZRepositories * )repositories{
    NSString *repoString = [NSString stringWithFormat:@"%@/%@",repositories.owner.login,repositories.name];
    ...
    NSRange range = NSMakeRange(repositories.owner.login.length + 1, repositories.name.length);
    [attrString addAttribute:NSFontAttributeName value:reposFont range:range];
    self.projectName.attributedText = attrString;
    
    NSURL *url = [NSURL URLWithString: repositories.owner.avatarURL];
    ...
    if (repositories.stargazersCount > 999) {
        double stars = repositories.stargazersCount / 1000.0;
        ...
        [self.starButton setTitle:[NSString stringWithFormat:@"%ld",(long)repositories.stargazersCount] forState:UIControlStateNormal];
    }
    
    
    if (repositories.forksCount > 999) {
        double forks = repositories.forksCount / 1000.0;
        ...
        [self.forkButton setTitle:[NSString stringWithFormat:@"%ld",(long)repositories.forksCount] forState:UIControlStateNormal];
    }
